...
export default class FripsShow extends React.Component {
  ...
  assignHotel(hotel) {
    ...
    const id = this.props.match.params.id
    axios.post(`/api/frips/${id}/hotels`, data, { headers: { Authorization: `Bearer ${Auth.getToken()}` } })
      ...
      <section>
        ...
        {frip && frip.hotels.map(hotel => (
          <div key={hotel.id}>
            ...
            <h4><a href={hotel.url} target='_blank' rel='noopener noreferrer'>{hotel.name}</a></h4>
        ...
        {hotels && hotels.result.map(hotel => (
          <div key={hotel.hotel_id}>
            ...
            <h6>{hotel.address_trans}, {hotel.city_trans}, {hotel.country_trans}</h6>
